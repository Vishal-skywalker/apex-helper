/**
 * NOTICE: THIS FILE HAS BEEN CHANGED FROM THE ORIGINAL VERSION PROVIDED BY THE ORIGINAL CREATOR OF THIS CODE
 */
...
/**
 * ApexFormatter is the auto-formatter implementation for Java.
 * @extends {AFormatter}
 */
export default class ApexFormatter extends Formatter {
  /**
   * Create a new ApexFormatter
   *
   * @constructor
  ...
  constructor(formatUnit) {
     ...
     * @private
     */
    this.methodSigRegex = new RegExp("^(public |private |protected |static |final |global " +
   ...
   */
  format(codeString) {
    const soqlPattern = /\[\s*SELECT\s*[^\]]*\s*\]/gi;
    const match = this.replaceAndExtract(soqlPattern, codeString);
    let data = this.formatSnippet(match.newText, null, null, null)
    data = this.putMatchesBack(data.join('\n'), match.matches);
    return data;
  }
   ...
   * @returns 
   */
  putMatchesBack(text, matches) {
    for (const [placeholder, match] of Object.entries(matches)) {
   ...
   * @returns 
   */
  replaceAndExtract(pattern, text) {
    let matchDict = {};
    let matchNumber = 1;

    // Replace the matches in the text
    const newText = text.replace(pattern, (match) => {
      const placeholder = `soqlMatch(${matchNumber})`;
      matchDict[placeholder] = match;
      matchNumber++;
      return placeholder;
    });

    return { newText, matches: matchDict };
  }
   ...
   * - An empty line (e.g. '')
   *
   * @param {String[]} An array of lines of code.
   * @param {Number} The index of the relevant line in the code array.
  ...
  _expressionIdentifier(codeArray, index) {
    if (codeArray.length > index) {
      ...
      return this._expressionIdentifier2(line) || this._midLineComment(line)
    }
   ...
   * @returns {boolean}
   */
  _midLineComment(line) {
    if (line && line.indexOf('//') !== -1) {
      return this._expressionIdentifier2(line.split('//')[0]?.trim() ?? '')
    }
    return false
  }
